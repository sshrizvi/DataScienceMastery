# <picture><source srcset="https://pandas.pydata.org/static/img/pandas_mark_white.svg" type="image/webp"><img src="https://pandas.pydata.org/static/img/pandas_mark_white.svg" width="32" height="32"></picture> Pandas for Data Science

> [!TIP]  
> Link to Previous Article  
> 🡸 [Data Visualization - Matplotlib](../Articles/123_matplotlib.md)

## Advanced Matplotlib

> [!IMPORTANT]  
> Link to notebook of this article for practical implementation.  
> [Data Visulaization - Matplotlib](../Notebooks/124_advanced_matplotlib.ipynb)



> [!IMPORTANT]  
> If you have studied Article 124, I would suggest you to perform some task so that you can check on your learning. Here is the link : [Task 24](/DataVisualization/Tasks/task_24.ipynb)

> [!TIP]  
> Link to Next Article  
> 🡺 [Session on Plotly (Express)](../Articles/125_plotly_express.md)